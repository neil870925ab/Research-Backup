...
def generate_conditional(tokenizer, model, input, device):
    """
...
def t5_ig_single(
    ...
    emb_layer = model.get_input_embeddings()         # T5's shared embedding
    ...
    # 6. Apply IG formula: (x - x0) * avg_grad, sum over hidden_size dimension → token attribution
...
def main():
    ...
    for split in SPLIT:
        ...
        args.output_file = os.path.join(current_path, '../', 'generate_baseline_rationales', 'output', args.task, f'baseline_rationales_{split}_output_ig_t5.jsonl')
        ...
        with open(args.input_file, "r", encoding="utf-8") as fin:
            for idx, line in enumerate(tqdm(fin, desc=f"T5 IG attribution for {split} file", total=total_lines), start=0):
        ...
        fout.close()
        print(f"Saved T5 IG annotation to {args.output_file}")
        if error_indexes:
