...
def quick_test_from_test_dataset(tokenizer, args, test_dataset, k=10):
    ...
    # ψ
    psi_model = Seq2SeqLeakProbe(args.model_name_or_path, tokenizer=tokenizer)
...
def train_leak_probe(args, tokenizer, train_dataset, eval_dataset):
    ...
    psi_model = Seq2SeqLeakProbe(args.model_name_or_path, tokenizer=tokenizer).to(args.device)
...
if __name__ == "__main__":
    ...
    # load data
    args.train_file  = os.path.join(current_path, '../', 'generate_baseline_rationales', 'output', args.task, 'baseline_rationales_train_output_ig_t5.jsonl')
    args.train_file = os.path.normpath(args.train_file)
    args.val_file  = os.path.join(current_path, '../', 'generate_baseline_rationales', 'output', args.task, 'baseline_rationales_val_output_ig_t5.jsonl')
    args.val_file = os.path.normpath(args.val_file)
    args.test_file  = os.path.join(current_path, '../', 'generate_baseline_rationales', 'output', args.task, 'baseline_rationales_test_output_ig_t5.jsonl')
