...
from utils import load_data_ecqa, load_data_esnli, load_data_ecqa_filtered, load_data_ecqa_irm, load_data_esnli_irm
    ...
    def create_irm_dataloader():
        class QuestionBatchSampler(Sampler):
            def __init__(self, qid_list, num_envs):
                ...
                self.unique_qids = list(set(qid_list))
...
def log_train_step(args, global_step, erm_mean, irm_penalty_mean, batch_loss, leak_loss):
    ...
    if args.use_leak_probe == 1:
        ...
        parts.append(f"---lambda_Leak={args.leak_penalty_weight}")
...
#             examples = load_data_func[args.task](args, file_path, data_type=args.data_type, shuffle=True)
...
def train(args, train_dataset, model, tokenizer, eval_dataset=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # ==========================
            if args.use_irm == 1:
                batch_loss, erm_mean, irm_penalty_mean, leak_loss = get_irm_loss(args, batch, model, tokenizer, global_step, psi_model=psi_model if args.use_leak_probe == 1 else None)
            else:
                batch_loss, erm_mean, leak_loss = get_loss(args, batch, model, tokenizer, psi_model=psi_model if args.use_leak_probe == 1 else None)
        ...
        if not save_predictions:
            ...
            for batch in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    ...
                    batch_loss, _, _ = get_loss(args, batch, model, tokenizer, psi_model=None)
            ...
            try:
                ...
                generate = (
                    generate_conditional_eval
                    if "t5" in args.model_name_or_path.lower() or "bart" in args.model_name_or_path.lower()
                    else generate_regular_eval
                )
...
def generate_regular_eval(tokenizer, model, args, input, output, device):
    """
    Generate a sequence with models like GPT, GPT2, or XLNet (for evaluation)
    """
