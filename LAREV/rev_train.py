...
from utils import init_model, load_data_ecqa, load_data_esnli, load_data_ecqa_filtered, load_data_ecqa_irm, load_data_esnli_irm
...
load_data_func = {
    ...
    'ECQA_FILTERED': load_data_ecqa_filtered,
...
def main():
    ...
    parser.add_argument(
        ...
        default="regular",
        type=str,
        help="temp: b, regular: [r, b], filtered: filtered high-quality data",
    )
    parser.add_argument(
        '--seednum', 
        action='store_true', 
    ...
    parser.add_argument(
        ...
        default="synthetic",
        type=str,
        help="the origin of the filtered data",
    )
    parser.add_argument(
        "--ablation_study",
        default="descending",
        type=str,
        choices=["descending", "ascending", "random", "customized"],
        help="Sorting method: 'descending' (highest REV first), 'ascending' (lowest REV first), 'random' (random selection), 'customized' (customized selection)"
    )
    parser.add_argument(
        "--use_irm",
        default=0,
    ...
    parser.add_argument(
        ...
        default=500,
        type=int,
        help="Number of iterations to linearly anneal IRMv1 penalty from 0 to full weight"
    )
    parser.add_argument(
        "--use_revirm_filtered_data",
        default=0,
        type=int,
        help="Use REV-IRM filtered data or not, only applicable when data_type is 'filtered'",
    )
    parser.add_argument(
        "--use_leak_probe",
        default=0,
    ...
    # Load filtered high-quality synthetic data generated from rev_filtering.py
    elif args.data_type == 'filtered':
      if args.task == 'ECQA':
        if args.ablation_study:
          if args.use_revirm_filtered_data == 1:
            args.train_file = os.path.join(current_path, 'filtered_data', 'revirm_filtered_data', args.task+'_irm-'+args.model_name_or_path+'-penalty'+str(args.irm_penalty_weight)+'_'+args.split+'_'+args.ablation_study+'.jsonl')
          else:
            args.train_file = os.path.join(current_path, 'filtered_data', 'rev_filtered_data', args.task+'_'+args.model_name_or_path+'_'+args.split+'_'+args.ablation_study+'.jsonl')
        else:
            raise ValueError("Please specify the ablation study method")
        args.train_file = os.path.normpath(args.train_file)
        logger.info(f"Filtered training file: {args.train_file}")
        args.data_path = os.path.join(current_path, '../', '../', '../', 'dataset', 'ecqa', 'output')
        args.data_path = os.path.normpath(args.data_path)
        args.val_file = os.path.join(args.data_path, 'ecqa_val.csv')
        args.test_file = os.path.join(args.data_path, 'ecqa_test.csv')
    ...
    if args.do_train:
        if 'gpt' in args.model_name_or_path:
            #not yet done for gpt when using filtered data ---neil
