...
def get_loss(args, batch, model, tokenizer, psi_model=None):
    ...
    # print("=== Debug: Decoder Input vs Labels ===")
    # print(f"Decoder input (first sample): {tokenizer.decode(decoder_input_ids[0][decoder_input_ids[0] != args.pad_token_id], skip_special_tokens=False)}")
    # print(f"Labels (first sample): {tokenizer.decode(lm_labels[0][lm_labels[0] != -100], skip_special_tokens=False)}")
    # raise ValueError("Debugging stop")  # Stop execution after debug prints
    ...
    if args.use_leak_probe == 1 and psi_model is not None:
    ...
    total_loss = erm_mean - (args.leak_penalty_weight*leak_loss) # the lower the leak loss, more leakage the model has, so we subtract it 
...
def get_irm_loss(args, batch, model, tokenizer, global_step, psi_model=None):
    ...
    for e in unique_envs:
        ...
        # # Debug: Print the first sample in the batch. Only print the non-padding part
        # print("=== Debug: Decoder Input vs Labels ===")
        # print(f"Decoder input (first sample): {tokenizer.decode(decoder_input_ids_e[0][decoder_input_ids_e[0] != args.pad_token_id], skip_special_tokens=False)}")
        # print(f"Labels (first sample): {tokenizer.decode(lm_labels_e[0][lm_labels_e[0] != -100], skip_special_tokens=False)}")
        # raise ValueError("Debugging stop")  # Stop execution after debug prints
    ...
    total_loss = erm_mean + (irm_lambda*irm_penalty_mean) - (args.leak_penalty_weight*leak_loss) # the lower the leak loss, more leakage the model has, so we subtract it 
